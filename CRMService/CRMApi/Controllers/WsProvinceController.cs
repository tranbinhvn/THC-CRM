...
namespace CRMApi.Controllers
{
    ...
    public class WsProvinceController : Controller
    {
        private readonly CRMContext _context;
        private ProvinceService _bbService;

        public WsProvinceController(CRMContext context)
        {
            _context = context;
            _bbService = new ProvinceService(_context);
        ...
        public IActionResult Get(int page, int size)
        {
            if (page <= 0 || size <= 0)
            {
                return new ObjectResult(new List<ProvinceDto>());
            }
             int total = 0;
            var tProvinceList = _bbService.getPaging(page, size, out total);
            var resBody = new ResponeBodyModel<List<ProvinceDto>>();
            resBody.errorCode = "0";
            resBody.errorMessage = "";
            resBody.totalRecord = total;
            resBody.result = tProvinceList;
            return new ObjectResult(resBody);
        }
        ...
        {
            var resBody = new ResponeBodyModel<ProvinceDto>();
            resBody.errorCode = "0";
            resBody.errorMessage = "";
            resBody.totalRecord = 0;
        ...
        // POST api/values
        [HttpPost]
        public IActionResult Create([FromBody] ProvinceDto value)
        {
        ...
        // PUT api/values/5
        [HttpPut]
        public IActionResult Update([FromBody] ProvinceDto value)
        {
