...
namespace CRMApi.Controllers
{
    ...
    public class WsBankBranchController : Controller
    {
        private readonly CRMContext _context;
        private BankBranchService _bbService;

        public WsBankBranchController(CRMContext context)
        {
            _context = context;
            _bbService = new BankBranchService(_context);
        ...
        {
            var resBody = new ResponeBodyModel<List<BankBranchDto>>();
            resBody.errorCode = "0";
        ...
        public IActionResult Get(int page, int size)
        {
            if (page <= 0 || size <= 0)
            {
                return new ObjectResult(new List<BankBranchDto>());
            }
             int total = 0;
        ...
        {
            var resBody = new ResponeBodyModel<BankBranchDto>();
            resBody.errorCode = "0";
            resBody.errorMessage = "";
            resBody.totalRecord = 0;
        ...
        // PUT api/values/5
        [HttpPut]
        public IActionResult Update([FromBody] BankBranchDto value)
        {
